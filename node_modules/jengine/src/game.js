JEngine.Game = JEngine.Utils.Obj.extend({
  ...
  processPackageJSON: function processPackageJSON() {
    var self = this;
  ...
  getJSON: function getJSON(type, path, options) {
    var url;
    if (!options) {
      if (_.isObject(path)) {
        options = path;
  ...
  urlFor: function urlFor(type, path) {
    if (!path) {
      path = type;
      type = 'root'
    }
