JEngine.Scene = JEngine.Utils.Obj.extend({
  ...
  start: function start() {
    ...
    self.updateLoop = function updateLoop() {
    ...
    self.fixedUpdateLoop = function fixedUpdateLoop() {
  ...
  addGameObject: function addGameObject(go, manifest, position, rotation, scale) {
    if (!(go instanceof JEngine.GameObject)) {
      go = go.Create(manifest, position, rotation, scale);
    }
    this.gameObjects.push(go);
    return this;
  },

  removeGameObject: function removeGameObject(uid_or_go) {
    if (_.isString(uid_or_go)) {
      this.gameObjects = _.reject(this.gameObjects, function(go) {return go.uid === uid_or_go});
    } else {
      this.gameObjects = _.without(this.gameObjects, uid_or_go);
    }
    return this;
  }
}, {
